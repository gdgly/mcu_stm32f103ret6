...
enum XsMessageId
{
	XMID_Wakeup             = 0x3E,							//接收 MT上电发送
	XMID_WakeupAck          = 0x3F,							//发送，回复此消息，MT进入命令模式
	XMID_ReqDid             = 0x00,							//发送  读取设备ID
	XMID_DeviceId           = 0x01,							//接受  接收数据ID
	XMID_GotoConfig         = 0x30,							//发送  进入命令模式
	XMID_GotoConfigAck      = 0x31,							
	XMID_GotoMeasurement    = 0x10,							//发送  进入测量模式
	XMID_GotoMeasurementAck = 0x11,
	XMID_MtData2            = 0x36,
	XMID_ReqOutputConfig    = 0xC0,							//发送  读取状态设置
	XMID_SetOutputConfig    = 0xC0,							//发送 	状态设置
	XMID_OutputConfig       = 0xC1,
	XMID_Reset              = 0x40,							//发送  复位
	XMID_ResetAck           = 0x41,
	XMID_Error              = 0x42,							//错误标记
};
