...
//		}
    ...
    Task.task_tick_now[0] = HAL_GetTick();
		int ret = HAL_GPIO_ReadPin(GPIOC,GPIO_PIN_1);
		if(ret)
		{
			uint8_t buffer[512],data_hand[64];
			while(1)
			{
				int ret;
				ret = HAL_UART_Receive(&huart2,buffer,20,100);
				if(ret == HAL_OK)
				{
...
//				data_hand[3] = 0x01;
//				data_hand[4] = 0x02;
//				HAL_UART_Transmit_IT(&huart2,data_hand,5);				
			}
			
	
//								user_p.acc[0] = (float)((redatagram[cnt+1]<<8)|(redatagram[cnt]))/1000;
//								user_p.adc[0] =	(float)(((redatagram[cnt+27]<<8)|(redatagram[cnt+26]))&0xFFF)*3.3/4096;
//								user_p.adc[1] =	(float)(((redatagram[cnt+29]<<8)|(redatagram[cnt+28]))&0xFFF)*3.3/4096;
//								user_p.adc[2] =	(float)(((redatagram[cnt+31]<<8)|(redatagram[cnt+30]))&0xFFF)*3.3/4096;
//								user_p.adc[3] =	(float)(((redatagram[cnt+33]<<8)|(redatagram[cnt+32]))&0xFFF)*3.3/4096;
//								user_p.adc[4] =	(float)(((redatagram[cnt+35]<<8)|(redatagram[cnt+34]))&0xFFF)*3.3/4096;
//								printf("adc[0]=%f adc[1]=%f adc[2]=%f adc[3]=%f adc[4]=%f\r\n",user_p.adc[0],user_p.adc[1],user_p.adc[2],user_p.adc[3],user_p.adc[4]);
...
void Usart_SendString( uint8_t pUSARTx, uint8_t *str)
	...
	HAL_UART_Transmit(&huart2,buf,k,20);
...
char *Getstrfromhc05(uint32_t millisec)
{
		...
		{
			if(HAL_UART_Receive(&huart2,s,1,100)==HAL_OK)
			{
